resource "openstack_blockstorage_volume_v2" "wordpress" {
  name = "wordpress"
  size = "30"
...
resource "openstack_compute_instance_v2" "wordpress" {
  ...
  security_groups = ["app"]
  availability_zone = "${var.wordpress_zone}"

  network {
    name = "${var.provider_network}"
    fixed_ip_v4 = "${var.wordpress_ipv4}"
  }

  block_device {
    uuid = "${openstack_blockstorage_volume_v2.wordpress.id}"
    boot_index = 0
    source_type = "volume"
  ...
  connection {
    ...
    user = "root"
    password = "${var.pass}"
    host = "${openstack_compute_instance_v2.wordpress.network.0.fixed_ip_v4}"
  }
