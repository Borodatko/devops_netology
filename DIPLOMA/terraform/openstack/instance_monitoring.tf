resource "openstack_blockstorage_volume_v2" "monitoring" {
  name = "monitoring"
  size = "30"
...
resource "openstack_compute_instance_v2" "monitoring" {
  ...
  availability_zone = "${var.monitoring_zone}"

  network {
    name = "${var.provider_network}"
    fixed_ip_v4 = "${var.monitoring_ipv4}"
  }

  block_device {
    uuid = "${openstack_blockstorage_volume_v2.monitoring.id}"
    boot_index = 0
    source_type = "volume"
  ...
  connection {
    ...
    user = "root"
    password = "${var.pass}"
    host = "${openstack_compute_instance_v2.monitoring.network.0.fixed_ip_v4}"
  }
